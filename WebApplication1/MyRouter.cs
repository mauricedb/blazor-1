// From: https://github.com/aspnet/Blazor/blob/dev/src/Microsoft.AspNetCore.Blazor/Routing/Router.cs
...
namespace WebApplication1
{
    ...
    /// </summary>
    public class MyRouter : IComponent, IDisposable
    {
        ...
        protected virtual Type GetComponentTypeForPath(string locationPath)
        {
            ...
            var match = re.Match(locationPath);
            if (match.Success)
            {
                locationPath = "/moviedetails";
            }
            ...
            return FindComponentTypeInAssemblyOrReferences(AppAssembly, componentTypeName)
                ?? throw new InvalidOperationException($"{nameof(MyRouter)} cannot find any component type with name {componentTypeName}.");
        }
